...
- **今日新增**：Redis 缓存落地（4 种 Key，空列表不缓存，SCAN 模式删除）、知识库/条目编辑功能、知识库分页、侧边栏美化（暖棕配色+底部用户卡+Header 面包屑）、按钮动画 `.btn-glow`、`React.StrictMode` 去除（解决双调 API）
- **今日新增**：热度统计与可视化（ECharts 柱状图+环形图+概览卡片，全局页+单库 Tab，60s 自动刷新，检索命中即时清缓存）
- 剩余加分项：自动提炼知识、多模态录入、一键生成专家 Agent
